...
contract VWBLERC721 is Ownable, AbstractVWBLToken, ERC721Enumerable, IViewPermission {
    ...
    function mint(string memory _getKeyURl, bytes32 _documentId) public payable returns (uint256) {
        ...
        setVWBLInfo(_documentId, msg.sender, _getKeyURl, tokenId);
        // grant access control to nft, pay vwbl fee and register nft data to access control checker contract
     ...
     */
    function batchMint(string memory _getKeyURl, bytes32[] memory _documentIds) public payable returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](_documentIds.length);
        for (uint i = 0; i < _documentIds.length; i++) {
            uint256 tokenId = ++counter;
            tokenIds[i] = tokenId;
            setVWBLInfo(_documentIds[i], msg.sender, _getKeyURl, tokenId);
        }
        // batch grant access control to nft, pay vwbl fee and register nft data to access control checker contract
