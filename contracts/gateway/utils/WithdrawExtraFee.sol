...
contract WithdrawExtraFee is Ownable, ReentrancyGuard, IWithdrawExtraFee {
    uint256 public totalPendingExtraFee;
    mapping(address => uint256) public srcAddrToPendingExtraFee;
    ...
    function withdrawAllPendingToken(address srcAddress) public nonReentrant {
        require(
            srcAddress == msg.sender || srcAddrToRecipient[srcAddress] == msg.sender,
            "msg sender is invalid withdrawer"
        );
        uint256 pendingAmount = srcAddrToPendingExtraFee[srcAddress];
     ...
     * @param srcAddress The address who call payFee method of VWBLGatewayV2 contract.
     */
    function withdrawPendingToken(address srcAddress, uint256 withdrawAmount) public nonReentrant {
        require(
            srcAddress == msg.sender || srcAddrToRecipient[srcAddress] == msg.sender,
            "msg sender is invalid withdrawer"
        );
    ...
    function rmFromSrcAddresses(address rmSrcAddress) private {
        ...
        uint256 j = 0;
        for (uint256 i = 0; i < srcAddresses.length; i++) {
